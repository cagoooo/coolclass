...
### Added
- 💬 **D2 教師回饋表 `feedback.html`**：6 段問卷（身分/星級/常用工具/卡關/期望/推薦意願 + Email），「📋 複製到剪貼簿」「📧 用 Email 寄出」「📋 預留 Google Form 連結」三種收集方式。
- 📘 **D1 教師示範頁 `tutorial.html`**：32 工具一張卡，包含「使用情境一段話 + 3 步驟上手 + 立刻試試 CTA」。8 個分類 chip（晨間/上課/班務/親師/投影/活動/報表） + 即時搜尋（200ms debounce）。
- ✍️ **F3 學期評語生成 `evaluation.html`**：分析 5 個工具的學生資料（情緒/出勤/積點/行為觀察/閱讀紀錄）自動產出 5 種文風（溫暖鼓勵/客觀描述/成長導向/積極進取/制式正式）+ 3 種字數（短 50/中 80/長 120）的個性化評語草稿。可單獨生成 / 全班批次 / CSV 匯出。
- 🌐 **G3 SEO 優化**：新增 `sitemap.xml`（40 個 URL）+ `robots.txt`。30 個 HTML 全部補 `<meta name="description">`。`manifest.json` description 更新為 32 工具版。

### Changed
- 📦 `sw.js` CORE_ASSETS 加入 5 個新檔
- 🧭 `nav.js`「設定與輔助」加入 3 個新項目
- 🏠 `index.html` 新增 G 區「v4.1 推廣與進階輔助」三卡
